...
const pushSync = async (req, res) => {
  ...
  // Fetch this shop's features + shop registration timestamp
  const featResult = await query(
    `SELECT uf.can_sell, uf.can_purchase, uf.can_repair, uf.can_reports,
            ...
            uf.free_entries_used, uf.premium_expires_at,
  ...
  if (featResult.rows.length === 0) {
    return forbidden(res, 'Shop profile not setup.', 'SHOP_NOT_SETUP');
  }

  const flags = featResult.rows[0];
  const now = new Date();
  const premiumExpiresAt = flags.premium_expires_at ? new Date(flags.premium_expires_at) : null;
  const isPremium = premiumExpiresAt !== null && premiumExpiresAt > now;

  const shopCreatedAt = new Date(flags.shop_created_at);
  const daysActive = Math.floor((now - shopCreatedAt) / (1000 * 60 * 60 * 24));
  const freeDaysLimit = flags.free_days_limit;
  const freeDaysRemaining = Math.max(0, freeDaysLimit - daysActive);

  // Free tier check: only count truly NEW entries (not already synced ones)
  if (!isPremium) {
    // Check if free trial days have expired
    if (freeDaysRemaining <= 0) {
      return forbidden(
        res,
        `Your free trial period of ${freeDaysLimit} days has expired. Please upgrade to premium.`,
        'TRIAL_EXPIRED_TIME'
      );
    }
